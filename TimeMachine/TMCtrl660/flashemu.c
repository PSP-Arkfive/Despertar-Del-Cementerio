...
static int read_cptbl_ciso(void* arg, void* addr, u32 size, u32 offset);
...
static SceSSize FlashEmu_IoRead(SceIoIob *iob, void *data, SceSize len)
{
    ...
    if (fep->virtual)
    {
        int res = ciso_read(&g_ciso_file, data, len, fep->uni.virtualinfo.offset);
        return res;
    }
